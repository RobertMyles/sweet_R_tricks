...
Recently it's been noticed by some [sharp]() observers that you can run Python and R in the same RMarkdown document and that the objects from one are available to the other. For example:  
```{r}
library(reticulate)
```
...
:heart_eyes:  
[Source](https://twitter.com/benomial/status/978462177601490945?s=08)
